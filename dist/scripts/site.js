...
(function () {
  ...
  function renderCart() {
    ...
    if (list) list.innerHTML = items.map(function (i, k) {
      ...
      return '<div class="line"><img src="' + (i.image || '/assets/img/brand/placeholder.svg') + '" alt="" width="64" height="64"><div><b>' + i.name.replace(/</g, '&lt;') + '</b><div class="muted">' + [i.weight, i.price].filter(Boolean).join(' · ') + '</div></div>'
  ...
  document.addEventListener('click', function (e) {
    ...
    if (history.length > 1 && document.referrer && new URL(document.referrer).origin === location.origin) history.back(); else location.href = '/';
  });
